...
type
  TNoteForm = class(TWebForm)
    ...
    divNoteTitleHolder: TWebHTMLDiv;
    divNoteTitle: TWebHTMLDiv;
    btnNoteCancel: TWebButton;
    btnNoteSave: TWebButton;
    divCategoriesHolder: TWebHTMLDiv;
    comboCategories: TWebComboBox;
    procedure WebFormCreate(Sender: TObject);
    procedure btnNoteCancelClick(Sender: TObject);
    procedure btnNoteSaveClick(Sender: TObject);
...
procedure TNoteForm.btnNoteCancelClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TNoteForm.btnNoteSaveClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TNoteForm.comboCategoriesChange(Sender: TObject);
begin
  if comboCategories.ItemIndex > 0
  then divNoteTitle.HTML.Text := '<div style="width:100%; text-align:center;">Report '+comboCategories.Text+'</div>'
  else divNoteTitle.HTML.Text := '<div style="width:100%; text-align:center;">Report an Issue</div>';
